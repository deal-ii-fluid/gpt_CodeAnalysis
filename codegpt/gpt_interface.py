...
def confirm_send(prompt: str, max_tokens=16000, yes=False, silent=False):
...
def send_iffy_edit(
    ...
    response = openai.ChatCompletion.create(
        ...
        messages= [
            {"role":"system","content":"You are an export Fortran 77 and Finite Element Method(FEM) project developer."},
...
def send_normal_completion(prompt, max_tokens=16000, yes=False):
    max_tokens = confirm_send(prompt, max_tokens, yes=yes)
    typer.secho(f"max_tokens: {max_tokens}")
